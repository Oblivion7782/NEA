...
#     scramble = LabelFrame(root,padx = 35,pady = 5,text = "Scramble(Green Front White Top)")
...
#     #TITLE
#     Label(title,text = "RUBIK CUBE SOLVER",pady = 3).pack()#.grid(row = 0, column = 0)
...
#     ]
#     selectors[0].pack()#.grid(row=0,column=0,padx=5,pady=2)
#     selectors[1].pack()#.grid(row=0,column=1,padx=5,pady=2)
#     selectors[2].pack()#.grid(row=1,column=0,padx=5,pady=2)
#     selectors[3].pack()#.grid(row=1,column=1,padx=5,pady=2)
#     selectors[4].pack()#.grid(row=2,column=0,padx=5,pady=2)
#     selectors[5].pack()#.grid(row=2,column=1,padx=5,pady=2)

#     #SCRAMBLER
#     Button(scramble,width=2,text = "R",command=lambda: [moves.R(),show(selected)]).pack()#.grid(row = 0, column = 0)
#     Button(scramble,width=2,text = "L",command=lambda: [moves.L(),show(selected)]).pack()#.grid(row = 0, column = 1)
#     Button(scramble,width=2,text = "F",command=lambda: [moves.F(),show(selected)]).pack()#.grid(row = 0, column = 2)
#     Button(scramble,width=2,text = "B",command=lambda: [moves.B(),show(selected)]).pack()#.grid(row = 0, column = 3)
#     Button(scramble,width=2,text = "U",command=lambda: [moves.U(),show(selected)]).pack()#.grid(row = 0, column = 4)
#     Button(scramble,width=2,text = "D",command=lambda: [moves.D(),show(selected)]).pack()#.grid(row = 0, column = 5)
#     Button(scramble,width=2,text = "R'",command=lambda: [moves.r(),show(selected)]).pack()#.grid(row = 1, column = 0)
#     Button(scramble,width=2,text = "L'",command=lambda: [moves.l(),show(selected)]).pack()#.grid(row = 1, column = 1)
#     Button(scramble,width=2,text = "F'",command=lambda: [moves.f(),show(selected)]).pack()#.grid(row = 1, column = 2)
#     Button(scramble,width=2,text = "B'",command=lambda: [moves.b(),show(selected)]).pack()#.grid(row = 1, column = 3)
#     Button(scramble,width=2,text = "U'",command=lambda: [moves.u(),show(selected)]).pack()#.grid(row = 1, column = 4)
#     Button(scramble,width=2,text = "D'",command=lambda: [moves.d(),show(selected)]).pack()#.grid(row = 1, column = 5)
#     Button(scramble,width=2,text = "R2",command=lambda: [moves.R2(),show(selected)]).pack()#.grid(row = 2, column = 0)
#     Button(scramble,width=2,text = "L2",command=lambda: [moves.L2(),show(selected)]).pack()#.grid(row = 2, column = 1)
#     Button(scramble,width=2,text = "F2",command=lambda: [moves.F2(),show(selected)]).pack()#.grid(row = 2, column = 2)
#     Button(scramble,width=2,text = "B2",command=lambda: [moves.B2(),show(selected)]).pack()#.grid(row = 2, column = 3)
#     Button(scramble,width=2,text = "U2",command=lambda: [moves.U2(),show(selected)]).pack()#.grid(row = 2, column = 4)
#     Button(scramble,width=2,text = "D2",command=lambda: [moves.D2(),show(selected)]).pack()#.grid(row = 2, column = 5)


#     #OPTIONS
#     Button(options,width=10,text="RESET CUBE",command=lambda:reset(opt,mode)).pack()#.grid(row=0,column=0)
#     Button(options,width=17,text="GENERATE SOLUTION",command=lambda:solve(opt)).pack()#.grid(row=1,column=0)

#     #OUTPUT
#     opt = Entry(output,width=71,state = "readonly")
#     opt.pack()#.grid(row = 0, column = 0)

#     #EXIT
#     ex = Button(root,text = "EXIT",command = root.destroy)
#     ex.pack()#.grid(row = 5, column = 1,padx = 10,pady = 10,sticky = W+E)

#     #Cube Holder
#     holder = LabelFrame(dispcube,bg="black")
#     holder.pack()  #.grid(row = 1,column = 1, rowspan = 3, columnspan = 3)
...
# passwordStr = str("1!qQwe")

# if re.search('[0-9]',passwordStr) is None:
#     print("Yes")
# else:
#     print("no")
...
passwordStr = b"laskdjf"

correct = False

salt = bcrypt.gensalt()
hashed = bcrypt.hashpw(passwordStr, salt)

if bcrypt.checkpw(passwordStr, hashed):
    correct = True
else:
    pass

print(correct)
print(bcrypt.checkpw(passwordStr, hashed))